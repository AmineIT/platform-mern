...
import { 
        ...
        DELETE_JOB,
        PUBLISH_JOB,
        ARCHIVE_JOB } from './types'
import { returnErrors } from './errorActions'
import { tokenConfig } from '../actions/authActions'

export const getCompanyJobs = (companyID) => (dispatch, getState) => {
    axios.post('/jobs/company-jobs', {companyID}, tokenConfig(getState)).then(res => {
        dispatch({
...
export const updateJob = (values) => (dispatch, getState) => {
    ...
    axios.post(`/jobs/update-job/${id}`, values, tokenConfig(getState)).then(res => {
        dispatch({
            type: UPDATE_JOB,
            payload: res.data
        })
